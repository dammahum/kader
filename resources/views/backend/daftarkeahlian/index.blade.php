			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header">
						<strong class="card-title">Data Daftar Keahlian</strong>
						<a href ="{{ route('daftarkeahlian.create') }}" class="btn btn-primary float-right"><i class="menu-icon fa fa-plus"></i> Tambah Data</a>
					</div>
					<div class="card-body">
						<table id="bootstrap-data-table" class="table table-striped table-bordered">
							...
							<tbody>
								<?php $no = 1; ?>
								@foreach($daftarkeahlians as $daftarkeahlian)
								<tr>
									<td>{{ $no++ }}</td>
									<td>{{ $daftarkeahlian->nama }}</td>
									<td>
										<form action=" {{ route('daftarkeahlian.destroy', $daftarkeahlian->id) }} " method="post">
					             {{ csrf_field() }}
					             {{ method_field("DELETE") }}
					             <a href="{{ route('daftarkeahlian.edit', $daftarkeahlian->id) }}" class="btn btn-sm btn-primary">
