			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header">
						...
						<a href ="{{ route('jenisusaha.create') }}" class="btn btn-primary float-right"><i class="menu-icon fa fa-plus"></i> Tambah Data</a>
					</div>
					<div class="card-body">
						<table id="bootstrap-data-table" class="table table-striped table-bordered">
							...
							<tbody>
								...
								<tr>
									<td>{{ $no++ }}</td>
									<td>{{ $jenisusaha->nama }}</td>
									<td>
										<form action=" {{ route('jenisusaha.destroy', $jenisusaha->id) }} " method="post">
					             {{ csrf_field() }}
					             {{ method_field("DELETE") }}
					             <a href="{{ route('jenisusaha.edit', $jenisusaha->id) }}" class="btn btn-sm btn-primary">
