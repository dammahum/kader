			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header">
						<strong class="card-title">Data Jenis Usaha</strong>
						<a href ="{{ route('daftarkitab.create') }}" class="btn btn-primary float-right"><i class="menu-icon fa fa-plus"></i> Tambah Data</a>
					</div>
					<div class="card-body">
						<table id="bootstrap-data-table" class="table table-striped table-bordered">
							...
							<tbody>
								<?php $no = 1; ?>
								@foreach($daftarkitabs as $daftarkitab)
								<tr>
									<td>{{ $no++ }}</td>
									<td>{{ $daftarkitab->nama }}</td>
									<td>
										<form action=" {{ route('daftarkitab.destroy', $daftarkitab->id) }} " method="post">
					             {{ csrf_field() }}
					             {{ method_field("DELETE") }}
					             <a href="{{ route('daftarkitab.edit', $daftarkitab->id) }}" class="btn btn-sm btn-primary">
