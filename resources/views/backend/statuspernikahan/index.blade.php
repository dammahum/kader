			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header">
						<strong class="card-title">Data Status Pernikahan</strong>
						<a href ="{{ route('statuspernikahan.create') }}" class="btn btn-primary float-right"><i class="menu-icon fa fa-plus"></i> Tambah Data</a>
					</div>
					<div class="card-body">
						<table id="bootstrap-data-table" class="table table-striped table-bordered">
							...
							<tbody>
								...
								<tr>
									<td>{{ $no++ }}</td>
									<td>{{ $statuspernikahan->status }}</td>
									<td>
										<form action=" {{ route('statuspernikahan.destroy', $statuspernikahan->id) }} " method="post">
					             {{ csrf_field() }}
					             {{ method_field("DELETE") }}
					             <a href="{{ route('statuspernikahan.edit', $statuspernikahan->id) }}" class="btn btn-sm btn-primary">
