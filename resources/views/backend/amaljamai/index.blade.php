			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header">
						<strong class="card-title">Data Amal Jamai</strong>
						<a href ="{{ route('amaljamai.create') }}" class="btn btn-primary float-right"><i class="menu-icon fa fa-plus"></i> Tambah Data</a>
					</div>
					<div class="card-body">
						<table id="bootstrap-data-table" class="table table-striped table-bordered">
							...
							<tbody>
								<?php $no = 1; ?>
								@foreach($amaljamais as $amaljamai)
								<tr>
									<td>{{ $no++ }}</td>
									<td>{{ $amaljamai->nama }}</td>
									<td>
										<form action=" {{ route('amaljamai.destroy', $amaljamai->id) }} " method="post">
					             {{ csrf_field() }}
					             {{ method_field("DELETE") }}
					             <a href="{{ route('amaljamai.edit', $amaljamai->id) }}" class="btn btn-sm btn-primary">
