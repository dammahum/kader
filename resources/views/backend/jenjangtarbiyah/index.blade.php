			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header">
						<strong class="card-title">Data Jenjang Tarbiyah</strong>
						<a href ="{{ route('jenjangtarbiyah.create') }}" class="btn btn-primary float-right"><i class="menu-icon fa fa-plus"></i> Tambah Data</a>
					</div>
					<div class="card-body">
						<table id="bootstrap-data-table" class="table table-striped table-bordered">
							...
							<tbody>
								<?php $no = 1; ?>
								@foreach($jenjangtarbiyahs as $jenjangtarbiyah)
								<tr>
									<td>{{ $no++ }}</td>
									<td>{{ $jenjangtarbiyah->nama }}</td>
									<td>
										<form action=" {{ route('jenjangtarbiyah.destroy', $jenjangtarbiyah->id) }} " method="post">
					             {{ csrf_field() }}
					             {{ method_field("DELETE") }}
					             <a href="{{ route('jenjangtarbiyah.edit', $jenjangtarbiyah->id) }}" class="btn btn-sm btn-primary">
